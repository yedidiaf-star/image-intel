...
def analyze(images_data):
    ...
    dict_result["unique_cameras"] = list(cameras)
    dict_result["date_range"] = [sorted_dict(images_data)[0]["datetime"], sorted_dict(images_data)[-1]["datetime"]]
    dict_result["insights"] = [f"תמונות שצולמו ברדיוס 1{distance(images_data)}",]
    if len(dict_result["unique_cameras"]) > 2:
        dict_result["insights"].append(f"נמצאו {len(dict_result["unique_cameras"])} מכשירים  יתכן שהסוכן החליף מכשירים")
